...
class AudioClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, num_classes)                                 # linear classifier
    def forward(self, x):
        x = self.resnet(x)
        return x    
...
def train_model(model, dataloader, criterion, optimizer, scheduler, num_epochs, track='batch'):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(model.state_dict(), f'./audio_classification/models/model_checkpoint_e{epoch}.pth')

    plt.savefig('./audio_classification/plots/batch_loss_plot.png')

# ====================== Main ======================
if __name__ == "__main__":
    source = 'balanced_train_segments'
    dataset = AudioDataset(f'./audio_classification/data/{source}.csv', f'./audio_classification/sounds_{source}', quiet=False, display=False, reduced=False, target_length=600)
